 ...
 * AudioBuffer.
 *
 * SampleBuffer stores the samples in an interlaced fashion. A monaural buffer
 * is straight-forward, a stereo buffer, though, has the following layout:
 *   [LRLRLRLRLR...].  A quadraphonic layout can have a similiar layout.  We
 * can add more constants for channel-number as we support more advanced
 * formats.  A SampleBuffer's data is as long as the frame count multiplied by
...
class SampleBuffer
{
  public:
    ...
    /**
     * Seek the write position to the requested frame.
     * This function takes interleaving into account. The write position will
     * be at the first channel of the requested frames.  This function as
     * well as write() are short-lived. They will be phased out once we have
     * proper streaming, this is just an temporary optimization for
     * SampleBufferReaders
     * @param frame  The absolute frame to seek to */
    int seek (nframes_t frame);

    /**
     * Write data starting at the write position and advance the position.
     * This function takes interleaving into account. The write position will
     * be at the first channel of the requested frames.  This function as
     * well as write() are short-lived. They will be phased out once we have
     * proper streaming, this is just an temporary optimization for
     * SampleBufferReaders
     * @param ptr the input data, must be at least the size of the frames
     *            parameter multipled by the number of channels
