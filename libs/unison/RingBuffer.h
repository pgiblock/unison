...
class RingBuffer
{
  ...
  public:
    ...
    /**
     * Read from the ringbuffer without moving the read pointer.  A convenient
     * way to inspect the data in a continous fashion. The cost is that the
     * data must be copied into the destination buffer.  Caller must check the
     * return value and call this function again if necessary, or use the peek
     ...
     * beginning of the buffer if the end is reached.
     *
     * @param dest a pointer to a buffer where data read from the ringbuffer
     *        will go.
     * @param cnt the number of elements to read.
     *
     * @return true if the buffer contains enough data to fulfill the request */
    ...
    /**
     * Advance the read pointer by the specified number of elements.  The data
     * is not copied, and is lost forever.  This is useful to avoid a copy if a
     * reader determines that the next chunk of data can be ignored.
     *
